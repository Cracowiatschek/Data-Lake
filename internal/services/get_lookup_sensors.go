...
type GetLookupSensorsService struct {
	s3Client *s3.Client
	repo     silver.Env
}

func NewGetLookupSensorsService(dt string) GetLookupSensorsService {
	return GetLookupSensorsService{
		s3Client: s3.New(),
		repo:     silver.SetupReferencesSensorIds(dt),
	}
}

func (s *GetLookupSensorsService) Run() error {
	start := time.Now()
	manifest := repositories.NewManifestRepository(s.repo.Layer, s.repo.Entity, s.repo.Dt)
	sourceEnv := bronze.SetupSensors(s.repo.Dt)
	...
	if lastLookupDate != "" {
		lastLookupPath := repositories.PathJson(s.repo.Layer, s.repo.Entity, lastLookupDate, "sensorsList")
		for {
		...
		var rawSensorsIds schemas.SensorIds
		if err := json.Unmarshal(lastLookupData, &rawSensorsIds); err != nil {
			manifest.MarkFailed()
			return err
		}

		for _, sensor := range rawSensorsIds.SensorId {
			sensorIds = append(sensorIds, sensor) // insert extists ID
		}
	}

	for _, record := range sensorsRaw {
		for _, sensor := range record.Sensors {
			if !pkg.IntInSlice(sensor.SensorID, sensorIds) {
				sensorIds = append(sensorIds, sensor.SensorID) // only insert fresh ID
			}
		}
	}
	rawRecords := len(sensorsRaw)
	computedRecords := len(sensorIds)
	sensorsResult := schemas.SensorIds{
		SensorId: sensorIds,
	}

	path := repositories.PathJson(s.repo.Layer, s.repo.Entity, s.repo.Dt, "sensorsList")
	payload, err := json.MarshalIndent(sensorsResult, "", " ")
...
func (s *GetLookupSensorsService) CleanUp() (error, bool) {
	...
	batchPath := repositories.PathJson(s.repo.Layer, s.repo.Entity, s.repo.Dt, "sensorsList ")
...
func (s *GetLookupSensorsService) SearchLeatestExistingSourceDate(layer, entity string) string {
